...
export const getConfig = () => {
  ...
  if (!common.webUrl) {
    throw new Error('NEXT_PUBLIC_WEB_URL environment variable is not configured');
  }

  if (!mastra.mastraUrl) {
    throw new Error('NEXT_PUBLIC_MASTRA_URL environment variable is not configured');
  }
  ...
  if (!copilotKit.publicLicenseKey) {
    throw new Error('NEXT_PUBLIC_COPILOTKIT_LICENSE_KEY environment variable is not configured');
  }
